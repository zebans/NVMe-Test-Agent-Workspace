# Directive Send Field Reference
...
| `DPTR` | Data Pointer | Start of host-to-controller data buffer. | Buffer format depends on `DTYPE`/`DOPER`. | Directive payload transfer. |
| `CDW10` bits 31:00 | `NUMD` | Number of dwords to transfer. | Zero-based. | Transfer length. |
